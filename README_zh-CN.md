...
```go
previous := svc.SetMemoryService(shared) // 返回时已排空并关闭
svc.SetMemoryService(nil)                // 关掉记忆;运行照常,只是不再记
```

被换下来的那个是**故意**关掉的:它持有一个后台写入协程,里面还排着没落盘的记忆,只丢指针会**静默**地把它们弄丢。切换请在服务空闲时做——正跑到一半的运行可能从旧后端检索、往新后端写。可运行:`examples/memory-swap`。
