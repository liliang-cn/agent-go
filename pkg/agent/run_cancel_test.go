...
func TestRunRegistry_Concurrent(t *testing.T) {
	...
	for i := 0; i < 16; i++ {
		...
		go func() {
			defer wg.Done()
			for j := 0; j < 50; j++ {
				runCtx, _, release, _ := svc.registerRun(context.Background(), ActiveRun{RunID: "", SessionID: "session", TaskID: "task"})
				_ = runCtx
...
func TestRegisterRun_DuplicateIDKeepsBothCancellable(t *testing.T) {
	...
	firstCtx, _, releaseFirst, _ := svc.registerRun(context.Background(), ActiveRun{RunID: "dup", SessionID: "s", TaskID: "t"})
	defer releaseFirst()
	secondCtx, _, releaseSecond, _ := svc.registerRun(context.Background(), ActiveRun{RunID: "dup", SessionID: "s", TaskID: "t"})
	defer releaseSecond()
