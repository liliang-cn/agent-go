...
func TestServiceGetLLMByProviderAndModel(t *testing.T) {
	svc := &Service{}
...
func TestServiceGetLLMServiceByProviderAndModel(t *testing.T) {
	svc := &Service{}
	cfg := newPoolTestConfig(t)
	seedPoolDB(t, cfg,
		[]*store.LLMProvider{
...
func TestServiceSaveLLMPoolConfigPersistsEmbeddingModel(t *testing.T) {
	cfg := newPoolTestConfig(t)
...
func TestServiceSaveEmbeddingProviderUpdatesExistingProvider(t *testing.T) {
	cfg := newPoolTestConfig(t)
	seedPoolDB(t, cfg,
		[]*store.LLMProvider{
			{Name: "openai_local", BaseURL: "http://local.example/v1", Key: "x", ModelName: "gpt-oss", MaxConcurrency: 2, Capability: 5, Enabled: true},
...
func TestSetServiceReplacesSingleton(t *testing.T) {
	original := SetGlobal(nil)
	defer SetGlobal(original)

	first := &Service{}
	if previous := SetGlobal(first); previous != nil {
		t.Fatalf("expected no previous global pool, got %p", previous)
	}
	if got := Global(); got != first {
		t.Fatalf("expected first injected global pool, got %p", got)
	}

	second := &Service{}
	if previous := SetGlobal(second); previous != first {
...
func TestSetServiceNilClearsSingleton(t *testing.T) {
	original := SetGlobal(nil)
	defer SetGlobal(original)
	...
	if previous := SetGlobal(nil); previous != injected {
		t.Fatalf("expected cleared global pool to return injected instance %p, got %p", injected, previous)
	}

	recreated := Global()
	if recreated == nil {
		t.Fatal("expected GetService to recreate a singleton after clear")
	}
